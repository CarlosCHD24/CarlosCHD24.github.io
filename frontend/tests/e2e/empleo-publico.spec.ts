...
test("employment explorer loads the master dataset and filters results", async ({ page }) => {
  ...
  await expect(page.getByText("302", { exact: true }).first()).toBeVisible();
  ...
  await expect(page.getByText("271 de 302 resultados", { exact: true })).toBeVisible();
  await expect(page.getByRole("button", { name: "31 finalizadas ocultas" })).toBeVisible();

  await page.getByRole("button", { name: "Limpiar filtros" }).click();
  await expect(page.getByRole("button", { name: "Ocultar 31 finalizadas" })).toHaveAttribute("aria-pressed", "false");
  ...
  await expect(page.getByText("279 de 302 resultados", { exact: true })).toBeVisible();
